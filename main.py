...
from config import API_ID, API_HASH, API_KEY, SESSION_FILE, HOST, PORT
...
async def init_telegram():
    """Инициализация Telegram клиента с сессией"""
    try:
        await client.connect()

        if not await client.is_user_authorized():
            logger.error("❌ Сессия недействительна! Создайте user.session заново.")
...
async def submit_complaint(
    ...
    if success:
        return ComplaintResponse(success=True, message="Жалоба успешно отправлена")
    else:
